...
$sql = "SELECT tc.tarefa_id, tc.usuario_tarefa, tc.titulo, tc.data_criada, tc.data_final,
	...
	(SELECT CONCAT(`nome`, ' ', `sobrenome`) FROM usuario where id = tc.criado_por) as criador,
	(SELECT email from usuario where id = tc.criado_por) as criador_email
	from tarefas_criadas tc where tarefa_id = '$idGet' LIMIT 1";
$query = mysqli_query($conexao, $sql);

if(mysqli_num_rows($query) > 0){
	$array = mysqli_fetch_array($query);
	if($array['usuario_tarefa'] !== $userSession){
		LocationIndex();
	}
